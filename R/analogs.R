...
#       lat.index <- unlist(lapply(1:length(y.obs), function(x) which.min((y.obs[x] - y.pred)^2)))
      ...
      if (isTRUE(use.PCs)) {
            sim.mat <- lapply(1:length(sim.mat), function(x) t(t(pred$COMBINED[[1]]$EOFs) %*% t(sim.mat[[x]])))
      }
      pred <- NULL
      # Analog search      
      ...
      d.list <- lapply(1:length(sim.mat), function(x) {
            ...
            b <- apply(aux, 1, function(vec, n.neigh) {sort(vec, index.return = TRUE)$ix[1:n.neigh]}, n.neigh)
            return(b)
      })
      pred.mat <- NULL
      # Analog assignation
