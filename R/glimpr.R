...
glimpr <- function(obs, pred, sim, pr.threshold = .1) {
      ...
      pred.list <- suppressWarnings(lapply(1:length(cases), function(x) {
            mod.bin <- glm(ymat.bin[ ,cases[x]] ~ ., data = as.data.frame(modelPars$pred.mat), family = binomial(link = "logit"))
            sims.bin <- sapply(1:length(modelPars$sim.mat), function(i) {
                  pred <- predict(mod.bin, newdata = as.data.frame(modelPars$sim.mat[[i]]), type = "response")
            ...
            mod.gamma <- glm(ymat[ ,cases[x]] ~., data = as.data.frame(modelPars$pred.mat), family = Gamma(link = "log"), subset = wet)
            sims <- sapply(1:length(modelPars$sim.mat), function(i) {
                  unname(predict(mod.gamma, newdata = as.data.frame(modelPars$sim.mat[[i]]), type = "response") * sims.bin[ ,i])
            })
            return(unname(sims))
      }))
