...
# $URL:  $
# $Id:  $
# Requires "netcdfAll-4.3.jar" see: [https://www.meteo.unican.es/trac/meteo/wiki/SpecsEuporias/RPackage/Requirements]
# TODO: enable data inventory for stations in ASCII / netCDF
# -------------------------------------------------------------------
            ...
            for (i in 1:length(varNames)) {
                  ...
                  if (length(gridLevels) > 0) {
                        ...
                        for (k in 1:length(gridLevels) - 1) {
                              ...
                              }, error = function(er) { # Level does not exist
                                    err = NA
                                    return(err)
                              })
                              v = c(v, res)
...
## Examples
# 1. Stations dataset
# dataset = '/home/juaco/PROYECTOS/EUPORIAS/DATA/ncmlExamples/ECA_Iberia.ncml'
# station.inv <- dataInventory(dataset)
# str(station.inv)
# names(station.inv)
# names(station.inv$ClimVars)
# # 2. Reanalysis
# dataset = "/home/juaco/temp/NCEP/NCEP_levels.ncml"
# ncep.inv <- dataInventory(dataset)
# str(ncep.inv)
##### EXAMPLE
# # Requires previous authentication (see [https://www.meteo.unican.es/trac/meteo/wiki/SpecsEuporias/RPackage/Authentication])
# library(rJava)
# # Assumes that 'netcdfAll-4.3.jar' is in the current working directory
# .jinit("netcdfAll-4.3.jar") 
