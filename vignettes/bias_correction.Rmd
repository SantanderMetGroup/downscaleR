...
NCEP reanalysis model data will be used as predictors and test data (type `help(NCEP_Iberia_pr)` and `help(NCEP_Iberia_tas)`). In order to transform NCEP precipitation units from Kg/m2/s to mm, we use function `gridArithmetics`. 
...
Next the climatology of NCEP precipitation (x) and the VALUE stations (y) are plotted with `spatialPlot` (from package `visualizeR`). In order to visualize the VALUE stations in the same map, we might use the functionalities of package `sp` (here function `SpatialPoints`).
...
Parametric quantile mapping is based on the initial assumption that both observed and simulated intensity distributions are well approximated by a given distribution, therefore it uses the theorical instead of the empirical distribution. For instance, the use of the "gamma" distribution is described in  [Piani et al. 2010](http://link.springer.com/article/10.1007%2Fs00704-009-0134-9), and here an example of application:
...
```{r, message=FALSE, warning=FALSE, results= 'hide'}
# time series plotting for Igueldo station
VALUE.obs <- subsetGrid(y, station.id = "000234")
CDX.cal <- subsetGrid(cal, station.id = "000234")
CDX.hist <- interpGrid(x, getGrid(VALUE.obs))
CDX.raw <- interpGrid(newdata, getGrid(VALUE.obs))
...
Next the "eqm" and "scaling" methods of bias correction are applied, using different periods for calibration and simulation.
...
tercilePlot(CFS_Iberia_tas, obs = EOBS_Iberia_tas, year.target = 2002, color.pal = "ypb")# Raw skill
```

In order to show the spread of the spatial mean of the ensemble of bias correction methodologies we could use function `temporalPlot`.
